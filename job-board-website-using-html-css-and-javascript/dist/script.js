...
function initRangeSlider() {
    salaryRange.addEventListener('input', function() {
        ...
        salaryValue.tex         
        Content = `₹${value.toLocaleString('en-IN')}+`;
        filterJobs();
    });
}*/
function initRangeSlider() {

    // 👉 page load होताना default value दाखव
    salaryValue.textContent = `₹${parseInt(salaryRange.value).toLocaleString('en-IN')}+`;

    salaryRange.addEventListener('input', function() {

        const value = parseInt(this.value);

        salaryValue.textContent = `₹${value.toLocaleString('en-IN')}+`;
...
document.getElementById("post-job-btn").onclick = function () {
    document.querySelector("#login-modal h2").textContent = "Sign Up to JobConnect";
    document.querySelector("#login-form button").textContent = "Sign Up";
...
document.getElementById("login-btn").onclick = function () {
    document.querySelector("#login-modal h2").textContent = "Login to JobConnect";
...
document.addEventListener("click", function () {
  const forgot = document.querySelector("");
  const loginForm = document.querySelector(".login-form");

  if (!forgot || !loginForm) return;

  forgot.style.display =
    loginForm.classList.contains("active") ? "inline" : "none";
});
